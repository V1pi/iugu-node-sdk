...
class IuguMethods {
  private get url (): string { return 'https://api.iugu.com/v1' }
    ...
    createIuguMethod <T> (method: IuguMethod): ((data: string | object, urlParams: Map<string, string> | undefined) => Promise<T>) {
      // eslint-disable-next-line @typescript-eslint/no-this-alias
      const self = this
      return async function (data: string | object, urlParams: Map<string, string> | undefined): Promise<T> {
        if (!self.apiKey) {
          throw new Error('Você deve chamar setApiKey')
        }

        if ((urlParams !== undefined && !method.urlParams) || (urlParams !== undefined && !method.urlParams.every((param) => urlParams.get(param) != null))) {
